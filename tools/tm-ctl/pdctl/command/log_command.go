...
func logCommandFunc(cmd *cobra.Command, args []string) {
	...
	if len(args) == 2 {
		...
		_, err = doRequestSingleEndpoint(cmd, url, logPrefix, http.MethodPost, http.Header{"Content-Type": {"application/json"}, "TM-Allow-follower-handle": {"true"}},
