...
func (suite *regionTestSuite) TestSplitRegions() {
	...
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/server/api/splitResponses", fmt.Sprintf("return(%v)", newRegionID)))
	err := tu.CheckPostJSON(testDialClient, fmt.Sprintf("%s/regions/split", suite.urlPrefix), []byte(body), checkOpt)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/server/api/splitResponses"))
	suite.NoError(err)
}
...
func (suite *regionsReplicatedTestSuite) TestCheckRegionsReplicated() {
	...
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/server/api/mockPending", "return(true)"))
	err = tu.ReadGetJSON(re, testDialClient, url, &status)
	suite.NoError(err)
	suite.Equal("PENDING", status)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/server/api/mockPending"))
	// test multiple rules
	r1 = core.NewTestRegionInfo(2, 1, []byte("a"), []byte("b"))
