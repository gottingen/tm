...
func (suite *scheduleTestSuite) TestOriginAPI() {
	...
	suite.NoError(err)
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/schedulers/persistFail", "return(true)"))
	suite.NoError(tu.CheckPostJSON(testDialClient, addURL, body, tu.StatusNotOK(re)))
	suite.Len(rc.GetSchedulers(), 1)
	resp = make(map[string]interface{})
	suite.NoError(tu.ReadGetJSON(re, testDialClient, listURL, &resp))
	suite.Len(resp["store-id-ranges"], 1)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/schedulers/persistFail"))
	suite.NoError(tu.CheckPostJSON(testDialClient, addURL, body, tu.StatusOK(re)))
	suite.Len(rc.GetSchedulers(), 1)
	resp = make(map[string]interface{})
	suite.NoError(tu.ReadGetJSON(re, testDialClient, listURL, &resp))
	...
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/server/config/persistFail", "return(true)"))
	statusCode, err := apiutil.DoDelete(testDialClient, deleteURL)
	suite.NoError(err)
	suite.Equal(500, statusCode)
	suite.Len(rc.GetSchedulers(), 1)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/server/config/persistFail"))
	statusCode, err = apiutil.DoDelete(testDialClient, deleteURL)
	suite.NoError(err)
