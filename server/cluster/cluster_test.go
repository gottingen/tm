...
import (
	...
	"github.com/gottingen/tm/pkg/progress"
	"github.com/gottingen/tm/pkg/schedule"
	"github.com/gottingen/tm/pkg/schedule/filter"
	"github.com/gottingen/tm/pkg/schedule/labeler"
...
func TestConcurrentReportBucket(t *testing.T) {
	...
	wg.Add(1)
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/cluster/concurrentBucketHeartbeat", "return(true)"))
	go func() {
		defer wg.Done()
		cluster.processReportBuckets(bucket1)
	}()
	time.Sleep(100 * time.Millisecond)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/cluster/concurrentBucketHeartbeat"))
...
func TestConcurrentRegionHeartbeat(t *testing.T) {
	...
	var wg sync.WaitGroup
	wg.Add(1)
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/cluster/concurrentRegionHeartbeat", "return(true)"))
	go func() {
		defer wg.Done()
		cluster.processRegionHeartbeat(source)
	}()
	time.Sleep(100 * time.Millisecond)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/cluster/concurrentRegionHeartbeat"))
