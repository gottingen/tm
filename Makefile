# Build tm-server, tm-ctl, tm-recover
default: build
...
ROOT_PATH := $(shell pwd)
BUILD_BIN_PATH := $(ROOT_PATH)/output

build: tm-server tm-ctl tm-recover

tools: tm-tso-bench tm-heartbeat-bench regions-dump stores-dump
...
tm-server: ${PD_SERVER_DEP}
	CGO_ENABLED=$(BUILD_CGO_ENABLED) go build $(BUILD_FLAGS) -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -tags "$(BUILD_TAGS)" -o $(BUILD_BIN_PATH)/tm-server cmd/tm-server/main.go

tm-server-basic:
	SWAGGER=0 DASHBOARD=0 $(MAKE) tm-server

.PHONY: build tools tm-server tm-server-basic

# Tools

tm-ctl:
	CGO_ENABLED=0 go build -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -o $(BUILD_BIN_PATH)/tm-ctl tools/tm-ctl/main.go
tm-tso-bench:
	cd tools/tm-tso-bench && CGO_ENABLED=0 go build -o $(BUILD_BIN_PATH)/tm-tso-bench main.go
tm-recover:
	CGO_ENABLED=0 go build -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -o $(BUILD_BIN_PATH)/tm-recover tools/tm-recover/main.go
tm-analysis:
	CGO_ENABLED=0 go build -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -o $(BUILD_BIN_PATH)/tm-analysis tools/tm-analysis/main.go
tm-heartbeat-bench:
	CGO_ENABLED=0 go build -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -o $(BUILD_BIN_PATH)/tm-heartbeat-bench tools/tm-heartbeat-bench/main.go
simulator:
	CGO_ENABLED=0 go build -gcflags '$(GCFLAGS)' -ldflags '$(LDFLAGS)' -o $(BUILD_BIN_PATH)/tm-simulator tools/tm-simulator/main.go
regions-dump:
...
.PHONY: tm-ctl tm-tso-bench tm-recover tm-analysis tm-heartbeat-bench simulator regions-dump stores-dump
...
TM-PKG := github.com/gottingen/tm
...
TSO_INTEGRATION_TEST_PKGS := $(TM-PKG)/tests/server/tso
...
# todo: it will remove server/api,/tests and tso packages after daily CI integrate all verify CI.
	go list ./... | grep -v -E  "github.com/gottingen/tm/server/api|github.com/gottingen/tm/tests/client|github.com/gottingen/tm/tests/server/tso" > packages.list;\
