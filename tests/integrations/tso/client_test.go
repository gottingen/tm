...
// More details can be found in this issue: https://github.com/gottingen/tm/issues/4884
func (suite *tsoClientTestSuite) TestUpdateAfterResetTSO() {
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/updateAfterResetTSO", "return(true)"))
	oldLeaderName := suite.cluster.WaitLeader()
	err := suite.cluster.GetServer(oldLeaderName).ResignLeader()
	re.NoError(err)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/updateAfterResetTSO"))
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/delaySyncTimestamp", `return(true)`))
	...
	re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/tso/delaySyncTimestamp"))
}

func (suite *tsoClientTestSuite) TestRandomTransferLeader() {
	re := suite.Require()

	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/fastUpdatePhysicalInterval", "return(true)"))
	defer re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/fastUpdatePhysicalInterval", "return(true)"))
...
func TestMixedTSODeployment(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/fastUpdatePhysicalInterval", "return(true)"))
	defer re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/fastUpdatePhysicalInterval", "return(true)"))
	re.NoError(failpoint.Enable("github.com/gottingen/tm/client/skipUpdateServiceMode", "return(true)"))
	defer re.NoError(failpoint.Enable("github.com/gottingen/tm/client/skipUpdateServiceMode", "return(true)"))
