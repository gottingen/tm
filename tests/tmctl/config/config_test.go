...
func TestConfig(t *testing.T) {
	...
	tmctl.MustPutStore(re, svr, store)
	...
	output, err = tmctl.ExecuteCommand(cmd, args1...)
	re.NoError(err)
	...
	_, err = tmctl.ExecuteCommand(cmd, args2...)
	re.NoError(err)
	...
	_, err = tmctl.ExecuteCommand(cmd, args3...)
	re.NoError(err)
	re.NotEqual(svr.GetLabelProperty(), labelPropertyCfg)
	output, err = tmctl.ExecuteCommand(cmd, args1...)
	re.NoError(err)
	...
	for _, testCase := range testCases {
		...
		_, err = tmctl.ExecuteCommand(cmd, args1...)
		re.NoError(err)
		// read
		args2 = []string{"-u", pdAddr, "config", "show"}
		output, err = tmctl.ExecuteCommand(cmd, args2...)
		re.NoError(err)
		cfg = config.Config{}
		re.NoError(json.Unmarshal(output, &cfg))
		// judge
	...
	output, err = tmctl.ExecuteCommand(cmd, argsInvalid...)
	re.NoError(err)
...
func TestPlacementRules(t *testing.T) {
	...
	output, err := tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "enable")
	re.NoError(err)
	re.Contains(string(output), "Success!")

	// test show
	var rules []placement.Rule
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "show")
	re.NoError(err)
	...
	// test load
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "load", "--out="+fname)
	re.NoError(err)
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "save", "--in="+fname)
	re.NoError(err)

	// test show group
	var rules2 []placement.Rule
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "show", "--group=tm")
	re.NoError(err)
...
func TestPlacementRuleGroups(t *testing.T) {
	...
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "show", "tm")
	re.NoError(err)
	re.NoError(json.Unmarshal(output, &group))
	re.Equal(placement.RuleGroup{ID: "tm"}, group)

	// test set
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "set", "tm", "42", "true")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "set", "group2", "100", "false")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "set", "group3", "200", "false")
	re.NoError(err)
	re.Contains(string(output), "Success!")

	// show all
	var groups []placement.RuleGroup
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "show")
	re.NoError(err)
	...
	// delete
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "delete", "group2")
	re.NoError(err)
	re.Contains(string(output), "Delete group and rules successfully.")

	// show again
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "show", "group2")
	re.NoError(err)
	re.Contains(string(output), "404")

	// delete using regex
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "delete", "--regexp", ".*3")
	re.NoError(err)

	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-group", "show", "group3")
	re.NoError(err)
	re.Contains(string(output), "404")
}
...
func TestPlacementRuleBundle(t *testing.T) {
	...
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "get", "tm")
	re.NoError(err)
	...
	f, err := os.CreateTemp("/tmp", "tm_tests")
	re.NoError(err)
	fname := f.Name()
	f.Close()
	defer func() {
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "load", "--out="+fname)
	re.NoError(err)
	b, _ := os.ReadFile(fname)
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "set", "--in="+fname)
	re.NoError(err)
	...
	// test delete
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "delete", "tm")
	re.NoError(err)
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "delete", "--regexp", ".*f")
	re.NoError(err)
	...
	re.NoError(err)
	re.NoError(os.WriteFile(fname, b, 0600))
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "save", "--in="+fname)
	re.NoError(err)
	...
	re.NoError(err)
	re.NoError(os.WriteFile(fname, b, 0600))
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "placement-rules", "rule-bundle", "save", "--in="+fname, "--partial")
	re.NoError(err)
...
func TestReplicationMode(t *testing.T) {
	...
	check := func() {
		output, err := tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "show", "replication-mode")
		re.NoError(err)
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "replication-mode", "dr-auto-sync")
	re.NoError(err)
	conf.ReplicationMode = "dr-auto-sync"
	check()

	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "replication-mode", "dr-auto-sync", "label-key", "foobar")
	re.NoError(err)
	conf.DRAutoSync.LabelKey = "foobar"
	check()

	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "replication-mode", "dr-auto-sync", "primary-replicas", "5")
	re.NoError(err)
	conf.DRAutoSync.PrimaryReplicas = 5
	check()

	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "replication-mode", "dr-auto-sync", "wait-store-timeout", "10m")
	re.NoError(err)
...
func TestUpdateDefaultReplicaConfig(t *testing.T) {
	...
	output, err := tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "max-replicas", "2")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	checkMaxReplicas(2)
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "location-labels", "zone,host")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	...
	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "max-replicas", "3")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	checkMaxReplicas(3)
	checkRuleCount(3)

	output, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "location-labels", "host")
	re.NoError(err)
	re.Contains(string(output), "Success!")
	...
	_, err = tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "set", "max-replicas", "4")
	re.NoError(err)
	checkMaxReplicas(4)
	checkRuleCount(4)
	checkLocaltionLabels(1)
...
func TestPDServerConfig(t *testing.T) {
	...
	output, err := tmctl.ExecuteCommand(cmd, "-u", pdAddr, "config", "show", "server")
	re.NoError(err)
	var conf config.TMServerConfig
