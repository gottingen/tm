...
import (
	...
	ctl "github.com/gottingen/tm/tools/tm-ctl/tmctl"
...
func TestStore(t *testing.T) {
	...
	for _, store := range stores {
		tmctl.MustPutStore(re, leaderServer.GetServer(), store.Store.Store)
	}
	...
	tmctl.CheckStoresInfo(re, storesInfo.Stores, stores[:2])
	...
	tmctl.CheckStoresInfo(re, storesInfo.Stores, stores)
	...
	tmctl.CheckStoresInfo(re, []*api.StoreInfo{storeInfo}, stores[:1])
...
// https://github.com/gottingen/tm/issues/5024
func TestTombstoneStore(t *testing.T) {
	...
	defer cluster.Destroy()
	tmctl.MustPutRegion(re, cluster, 1, 2, []byte("a"), []byte("b"), core.SetWrittenBytes(3000000000), core.SetReportInterval(0, statistics.WriteReportInterval))
	tmctl.MustPutRegion(re, cluster, 2, 3, []byte("b"), []byte("c"), core.SetWrittenBytes(3000000000), core.SetReportInterval(0, statistics.WriteReportInterval))
	// store remove-tombstone
	args := []string{"-u", pdAddr, "store", "remove-tombstone"}
	output, err := tmctl.ExecuteCommand(cmd, args...)
	re.NoError(err)
