...
func TestRegionKeyFormat(t *testing.T) {
	...
	output, err := tmctl.ExecuteCommand(cmd, "-u", url, "region", "key", "--format=raw", " ")
	re.NoError(err)
...
func TestRegion(t *testing.T) {
	...
	r1 := tmctl.MustPutRegion(re, cluster, 1, 1, []byte("a"), []byte("b"),
	...
	r2 := tmctl.MustPutRegion(re, cluster, 2, 1, []byte("b"), []byte("c"),
	...
	r3 := tmctl.MustPutRegion(re, cluster, 3, 1, []byte("c"), []byte("d"),
	...
	r4 := tmctl.MustPutRegion(re, cluster, 4, 1, []byte("d"), []byte("e"),
	...
	for _, testCase := range testRegionsCases {
		...
		tmctl.CheckRegionsInfo(re, regions, testCase.expect)
	}
	...
	for _, testCase := range testRegionCases {
		...
		tmctl.CheckRegionInfo(re, region, testCase.expect)
	}

	// Test region range-holes.
	r5 := tmctl.MustPutRegion(re, cluster, 5, 1, []byte("x"), []byte("z"))
	output, err := tmctl.ExecuteCommand(cmd, []string{"-u", pdAddr, "region", "range-holes"}...)
	re.NoError(err)
