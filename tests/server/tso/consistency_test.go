...
func (suite *tsoConsistencyTestSuite) TestSynchronizedGlobalTSOOverflow() {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/globalTSOOverflow", `return(true)`))
	suite.getTimestampByDC(ctx, cluster, tso.GlobalDCLocation)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/tso/globalTSOOverflow"))
}
...
func (suite *tsoConsistencyTestSuite) TestLocalAllocatorLeaderChange() {
	...
	defer cancel()
	suite.getTimestampByDC(ctx, cluster, tso.GlobalDCLocation)
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/server/mockLocalAllocatorLeaderChange"))
}
...
func (suite *tsoConsistencyTestSuite) TestLocalTSOAfterMemberChanged() {
	...
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/tso/systemTimeSlow"))
}
