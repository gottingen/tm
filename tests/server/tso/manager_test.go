...
func TestNextLeaderKey(t *testing.T) {
	...
	defer cluster.Destroy()
	re.NoError(err)
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/tso/injectNextLeaderKey", "return(true)"))
	re.NoError(cluster.RunInitialServers())
	...
	re.Equal("", originName)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/tso/injectNextLeaderKey"))
	cluster.CheckClusterDCLocation()
