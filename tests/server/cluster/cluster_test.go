...
import (
	...
	"github.com/gottingen/tm/pkg/core/storelimit"
	"github.com/gottingen/tm/pkg/dashboard"
	"github.com/gottingen/tm/pkg/id"
	"github.com/gottingen/tm/pkg/mock/mockid"
	"github.com/gottingen/tm/pkg/schedule/operator"
	"github.com/gottingen/tm/pkg/storage"
	...
	syncer "github.com/gottingen/tm/server/region_syncer"
...
func TestStaleRegion(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/cluster/decEpoch", `return(true)`))
	tc.HandleRegionHeartbeat(regionInfoA)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/cluster/decEpoch"))
	regionInfoA = regionInfoA.Clone(core.WithIncConfVer(), core.WithIncVersion())
	err = tc.HandleRegionHeartbeat(regionInfoA)
	re.NoError(err)
}
...
func TestStoreVersionChange(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/versionChangeConcurrency", `return(true)`))
	wg.Add(1)
	go func() {
		defer wg.Done()
		resp, err := putStore(grpcPDClient, clusterID, store)
	...
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/versionChangeConcurrency"))
}
...
func TestSetScheduleOpt(t *testing.T) {
	...
	// PUT GET failed
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/storage/kv/etcdSaveFailed", `return(true)`))
	...
	// DELETE failed
	re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/storage/kv/etcdSaveFailed"))
	re.NoError(svr.SetReplicationConfig(*replicationCfg))
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/storage/kv/etcdSaveFailed", `return(true)`))
...
// See https://github.com/gottingen/tm/issues/4941
