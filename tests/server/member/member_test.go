...
func TestLeaderResignWithBlock(t *testing.T) {
	...
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/raftclusterIsBusy"))
}
...
func TestPDLeaderLostWhileEtcdLeaderIntact(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/leaderLoopCheckAgain", fmt.Sprintf("return(\"%d\")", memberID)))
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/exitCampaignLeader", fmt.Sprintf("return(\"%d\")", memberID)))
	re.NoError(failpoint.Enable("github.com/gottingen/tm/server/timeoutWaitPDLeader", `return(true)`))
	leader2 := waitLeaderChange(re, cluster, leader1)
	re.NotEqual(leader1, leader2)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/leaderLoopCheckAgain"))
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/exitCampaignLeader"))
	re.NoError(failpoint.Disable("github.com/gottingen/tm/server/timeoutWaitPDLeader"))
}
