...
import (
	...
	"github.com/gottingen/tm/pkg/cache"
	...
	"github.com/gottingen/tm/pkg/utils/operatorutil"
...
// Ref https://github.com/gottingen/tm/issues/3521 https://github.com/gottingen/tm/issues/5786
...
func (suite *ruleCheckerTestSuite) TestIssue3293() {
	...
	suite.NoError(err)
	err = suite.ruleManager.DeleteRule("tm", "default")
	suite.NoError(err)
	op := suite.rc.Check(suite.cluster.GetRegion(1))
	suite.NotNil(op)
	suite.Equal("add-rule-peer", op.Desc())
...
// See issue: https://github.com/gottingen/tm/issues/3705
...
func (suite *ruleCheckerTestSuite) TestRuleCache() {
	...
	for _, testCase := range testCases {
		...
		if testCase.stillCached {
			suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/checker/assertShouldCache", "return(true)"))
			suite.rc.Check(testCase.region)
			suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/checker/assertShouldCache"))
		} else {
			suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/checker/assertShouldNotCache", "return(true)"))
			suite.rc.Check(testCase.region)
			suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/checker/assertShouldNotCache"))
		}
	}
}

// Ref https://github.com/gottingen/tm/issues/4045
...
// Ref https://github.com/gottingen/tm/issues/4140
func (suite *ruleCheckerTestSuite) TestDemoteVoter() {
	...
	suite.ruleManager.DeleteRule("tm", "default")
