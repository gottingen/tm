...
func (suite *operatorControllerTestSuite) SetupSuite() {
	...
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/unexpectedOperator", "return(true)"))
}
...
func (suite *operatorControllerTestSuite) TestCheckAddUnexpectedStatus() {
	suite.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/unexpectedOperator"))
...
func (suite *operatorControllerTestSuite) TestConcurrentRemoveOperator() {
	...
	suite.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/concurrentRemoveOperator", "return(true)"))
...
func (suite *operatorControllerTestSuite) TestInvalidStoreId() {
	...
	stream := hbstream.NewTestHeartbeatStreams(suite.ctx, tc.ID, tc, false /* no need to run */)
	oc := NewOperatorController(suite.ctx, tc, stream)
	// If TM and store 3 are gone, TM will not have info of store 3 after recreating it.
	...
	// Although store 3 does not exist in TM, TM can also send op to TiKV.
	suite.Equal(pdpb.OperatorStatus_RUNNING, oc.GetOperatorStatus(1).Status)
}
