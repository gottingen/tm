...
func scatterSpecial(re *require.Assertions, numOrdinaryStores, numSpecialStores, numRegions uint64) {
	...
	re.NoError(tc.RuleManager.SetRule(&placement.Rule{
		GroupID: "tm", ID: "learner", Role: placement.Learner, Count: 3,
...
func TestScatterForManyRegion(t *testing.T) {
	...
	group := "group"
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/scatterHbStreamsDrain", `return(true)`))
	scatterer.scatterRegions(regions, failures, group, 3)
	re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/scatterHbStreamsDrain"))
	re.Len(failures, 0)
}
...
func TestScattersGroup(t *testing.T) {
	...
	for id, testCase := range testCases {
		...
		failures := map[uint64]error{}
		if testCase.failure {
			re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/schedule/scatterFail", `return(true)`))
		}
		...
		if testCase.failure {
			...
			_, ok := failures[1]
			re.True(ok)
			re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/schedule/scatterFail"))
		} else {
			re.Empty(failures)
		}
	}
...
// Ref https://github.com/gottingen/tm/issues/4565
...
// Ref https://github.com/gottingen/tm/issues/5909
...
// ref https://github.com/gottingen/tm/issues/6017
func TestBalanceRegion(t *testing.T) {
	...
	// ref https://github.com/gottingen/tm/issues/6099
