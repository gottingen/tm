...
func TestLoadRegionsExceedRangeLimit(t *testing.T) {
	re := require.New(t)
	re.NoError(failpoint.Enable("github.com/gottingen/tm/pkg/storage/kv/withRangeLimit", "return(500)"))
	storage := NewStorageWithMemoryBackend()
	cache := core.NewBasicCluster()
	...
	re.NoError(failpoint.Disable("github.com/gottingen/tm/pkg/storage/kv/withRangeLimit"))
}
