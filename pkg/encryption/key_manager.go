...
import (
	...
	"github.com/gottingen/tm/pkg/election"
...
type Manager struct {
	...
	mu struct {
		syncutil.Mutex
		// TM leadership of the current TM node. Only the TM leader will rotate data keys,
...
func saveKeys(
	...
	// Avoid write conflict with TM peer by checking if we are leader.
...
func (m *Manager) checkOnTick() {
	m.mu.Lock()
	defer m.mu.Unlock()
	// Check data key rotation in case we are the TM leader.
...
func (m *Manager) GetKey(keyID uint64) (*encryptionpb.DataKey, error) {
	...
	// The key could be generated by another TM node, which shouldn't happen normally.
	m.mu.Lock()
	defer m.mu.Unlock()
...
// SetLeadership sets the TM leadership of the current node. TM leader is responsible to update
